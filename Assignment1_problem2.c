...
int pid=fork(); // system call for creation
...
printf("\nI'm Child with pid = %d and My parent's pid = %d  \n",getpid(),getppid());

array[0][0]=4; // changing to invalid value

// print the matrix "child version".
printf("\nThe matrix in child version :\n");printf("\n");
...
exit(0);//send parameter to inform wait to complete.
}

//======================parent process=========================

else{
wait(NULL); // will excute the child process first
printf("\n<><><><><><><><><><><><><><><><><><><><><><><><><><><><><><>");
printf("\n\nI'm Parent with pid = %d \n",getpid());
// print the matrix "parent version"
printf("\nThe matrix in parent version :\n");printf("\n");
for(int i=0 ; i<size ; i++){//nested loop for printing the matrix
for(int j=0 ; j<size ; j++){
printf("%d , ",array[i][j]);
}
...
pthread_create(&thread1,NULL,row,i);// i indicate number of row starting with 0.
pthread_join(thread1,NULL);// pass to row runner.
}
...
for(int i=0;i<size;i++){// loop for each column.
pthread_create(&thread2,NULL,column,i);// i indicate number of column starting with 0.
pthread_join(thread2,NULL);// pass to column runner.
}
//-----------------checking latin matrix-------------------
if(result>0)// final result for validity of entire matrix
